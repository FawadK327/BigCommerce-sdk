  ...
  el.onload = function(){                      //After the scrip loaded function starts
    ...
    .then(function (state) {
      ...
      function qisstpay_open_checkout_cart() {
        ...
        var discountGiven = checkoutObj.coupons.length ? checkoutObj.coupons[0].discountedAmount : 0;   //Getting the discount after coupon Applied
        // console.log('ShippingMethod : ',shipping_method);
        cartObj = checkoutObj.cart;    //Cart object including all the details of cart
        
        if(cartObj){
          if(discountGiven == 0){
            discountGiven = cartObj.discountAmount; //If there is no coupon applied
          }
          ...
          var totalPrice = cartObj.cartAmount;   //Cart total price after discount and shippingCost
